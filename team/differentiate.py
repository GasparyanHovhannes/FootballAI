...
def differentiate_by_color(
    ...
    """
    Assign players to attacking/defending by jersey color. Pressure is measured on
    the player with the ball; attacking = team with the ball, defending = team applying pressure.

    - If attacking_team is None and the ball is detected: the player closest to the ball
      is the ball carrier; their cluster = attacking (team with the ball).
    - If attacking_team is "home" or "away": that team is the one with the ball (home = right
      side of pitch, away = left, by mean x of clusters).

    Use this when you have the image path; falls back to spatial differentiate if
    fewer than 2 players (then attacking_team is required).
    """
